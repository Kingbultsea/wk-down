...
class Share {
    constructor (userId, accessToken, platFormId, unionid, nickName, picUrl, allUserData) {
        this.userId = userId
    ...
    async addOwner (type = 'null') {
        ...
        if (judge) {
            ...
            id = await db2.insert('mp_user', {
                openid: this.allUserData.openid,
                unionId: this.unionId,
                nickname: this.allUserData.nickname,
                headimgurl: this.allUserData.headimgurl,
                sex: this.allUserData.sex,
                country: this.allUserData.country,
                province: this.allUserData.province,
                city: this.allUserData.city,
                language: this.allUserData.language,
                subscribe: this.allUserData.subscribe,
                subscribe_time: this.allUserData.subscribe_time,
                groupid: this.allUserData.groupid,
                created_at: ('' + new Date().getTime()).substr(0, 10),
                updated_at: ('' + new Date().getTime()).substr(0, 10)
            })
        }
        const hash = await this.randomString(6)
        const insertData = {
            unionid: this.unionId,
        ...
        await db.insert(`owner`, insertData)
